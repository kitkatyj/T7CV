...
class UNetWithInception(nn.Module):
    def __init__(self, block, n_inputs, n_outputs, batchnorm=False, joinType="concat", upmode="transpose"):
        ...
        # Adjust the output channels of the "inception" block to match decoder's expected input channels (256)
        self.inception = InceptionModule(nf[0], nf[2], batchnorm=batchnorm)

        self.decoder = nn.Sequential(
            Conv_3d(nf[0], nf[1], kernel_size=3, padding=1, bias=True, batchnorm=batchnorm),
            upConv3D(nf[1] * growth, nf[2], kernel_size=(3, 4, 4), stride=(1, 2, 2), padding=(1, 1, 1), upmode=upmode,
                     batchnorm=batchnorm),
            upConv3D(nf[2] * growth, nf[3], kernel_size=(3, 4, 4), stride=(1, 2, 2), padding=(1, 1, 1), upmode=upmode,
                     batchnorm=batchnorm),
            Conv_3d(nf[3] * growth, nf[3], kernel_size=3, padding=1, bias=True, batchnorm=batchnorm),
            upConv3D(nf[3] * growth, nf[3], kernel_size=(3, 4, 4), stride=(1, 2, 2), padding=(1, 1, 1), upmode=upmode,
                     batchnorm=batchnorm)
        )
    ...
    def forward(self, images):
        ...
        x_0, x_1, x_2, x_3, x_4 = self.encoder(images)
        x_4 = self.inception(x_4)
        dx_3 = self.lrelu(self.decoder[0](x_4))

        # Apply joinTensors operation here
        dx_3 = joinTensors(dx_3, x_3, type=self.joinType)
        dx_2 = self.lrelu(self.decoder[1](dx_3))
